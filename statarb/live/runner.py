...
class RunnerConfig:
    ...
    poll_seconds: int = 30
    max_drawdown: float = 0.20        # halt if equity falls this far from peak
...
def run_live(
    ...
    # ---- startup checklist ----
    log.info("=== statarb %s starting ===", mode)
    log.info("strategy: %s | symbols: %s | tf: %s", strategy.name, cfg.symbols, cfg.timeframe)

    if hasattr(broker, "reconcile_on_startup"):
        broker.reconcile_on_startup(cfg.symbols)

    prices_init = _latest_prices(provider, cfg.symbols, cfg.timeframe)
    eq0 = broker.equity(prices_init)
    state.peak_equity = eq0
    alerter.alert_startup(strategy.name, mode, cfg.symbols)
    ...
    while True:
        ...
        try:
            ...
            closes = panel_closes(provider, cfg.symbols, cfg.timeframe, start=start, end=end)
...
def _latest_prices(provider: CCXTProvider, symbols: list[str], timeframe: str) -> dict[str, float]:
    closes = panel_closes(provider, symbols, timeframe,
