...
def _make_strategy_factory(use_kalman, entry, exit_, stop, lookback):
    ...
    def factory(pair: PairSpec):
        return PairsStrategy(pair, strat_cfg)
    return factory


def _build_pair_manager(provider, exchange, quote, timeframe, slots, top, scan_days, rescan_days,
                        pvalue, use_kalman, entry, exit_, stop, lookback):
    from .live.pair_manager import PairManager, PairManagerConfig
    pm_cfg = PairManagerConfig(
        n_slots=slots, universe_top=top, quote=quote,
        scan_days=scan_days, rescan_interval_days=rescan_days, pvalue_max=pvalue,
    )
    factory = _make_strategy_factory(use_kalman, entry, exit_, stop, lookback)
    return PairManager(provider, pm_cfg, timeframe, factory)
...
def paper(
    ...
    slots: int = typer.Option(1, "--slots", help="number of parallel pair slots (auto-scan mode)"),
...
def live(
    ...
    fixed_mode = bool(base and x and slots == 1)
    pair_label = f"{base} / {x}" if fixed_mode else f"auto-scan  {slots} slot(s)  top-{top}"
